...
import RampCreateTransferInAddress from '../ramps/endpoints/Ramp.createTransferInAddress';
import USDCAddress from '../ramps/models/USDCAddress';
import { setupDb, stop } from './_setup';
import config from '../config';
import USDCApi from '../ramps/lib/USDCApi';
import USDCMonitor from '../ramps/lib/USDCMonitor';
import TestClient from './_TestClient';
import AddressGetBalance from '../main/endpoints/Address.getBalance';
import BlockManager from '../main/lib/BlockManager';
import RampDb, { setup } from '../ramps/db';
import MainDb from '../main/db';
import ConsumerPriceIndex, { IConsumerPriceIndex } from '../main/models/ConsumerPriceIndex';
import USDCTransfer from '../ramps/models/USDCTransfer';
import EthereumHDWallet from '../ramps/lib/EthereumHDWallet';
import { USDCNetworks } from '../ramps/lib/USDCNetworks';
import SidechainAudit, { ISidechainAudit } from '../ramps/models/SidechainAudit';
import RampAudit from '../ramps/endpoints/Ramp.audit';
import validateAudit from '../ramps/scripts/validateAudit';
...
describe('USDC -> Argons', () => {
  ...
  test('should convert funds to Argons after enough confirmations', async () => {
    ...
    jest.spyOn(SidechainAudit, 'latestSignedAudit').mockImplementationOnce(async () => {
      return {
        auditDate: new Date(),
    ...
    config.ramp.sidechainAddressesForReserves = [reservesClient.credentials.address];
...
describe('Sidechain Audits', () => {
  ...
  test('should be able to audit the holdings of a Sidechain', async () => {
    ...
    const audit = await SidechainAudit.audit({});
  ...
  test('should be able to sign audits proving ownership', async () => {
    ...
    const audit = await RampAudit.handler(undefined, { logger: null });
