...
export default class MicronoteBatchClose {
  ...
  constructor(readonly client: PgClient<DbType.Batch>, readonly batch: MicronoteBatch) {
  ...
  private signLedgerOutputs(
    ...
    return Note.addSignature(
      {
        toAddress,
        centagons,
        fromAddress: this.batch.address,
  ...
  private async findOrphanedFunding(defaultClient: PgClient<DbType.Default>): Promise<void> {
    ...
    const promises = this.missingHashes.map(async hash => {
      const note = await Note.load(hash, this.logger);
  ...
  private async refundUnclaimedNotes(): Promise<void> {
    ...
    const promises = this.unclaimedNotes.map(async note => {
      const funding = new MicronoteFunds(this.client, this.batch, note.clientAddress);
