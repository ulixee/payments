...
export default class SidechainAudit {
  public static table = 'sidechain_audits';

  public data: ISidechainAudit;

  public static async latestSignedAudit(client: PgClient<DbType.Ramp>): Promise<ISidechainAudit> {
    const result = await client.query<ISidechainAudit>(
      `select * from ${this.table} where signatures_complete_date is not null order by audit_date desc LIMIT 1`,
    );
    return result.rows[0];
  }

  public static async audit(opts: ITransactionOptions): Promise<ISidechainAudit> {
    const usdcToArgonConversionRate = await BridgeToMain.getUsdToArgonConversionRate();
    const centagonsInCirculation = await BridgeToMain.totalCentagonsInCirculation(opts);
    const auditDate = moment().startOf('day').toDate();

    return RampDb.transaction(async client => {
      const existing = await client.query<ISidechainAudit>(
        `select 1 from ${this.table} where audit_date=$1`,
        [auditDate],
      );
      if (existing.rows.length) return existing.rows[0];

      const reserveWallets = config.ramp.neuteredHDWalletsForReserves;
      ...
      return client.insert<ISidechainAudit>(this.table, {
        auditDate,
        usdcAddresses: reserveWallets.map(
          x =>
            <ISidechainAudit['usdcAddresses'][0]>{
              address: x.address,
              nodePath: x.path,
              ...x.meta,
            },
  ...
  public static async signAudits(wallet: EthereumHDWallet<any>): Promise<ISidechainAudit[]> {
    return await RampDb.transaction(async client => {
      const needingSignatures = await client.list<ISidechainAudit>(
      ...
      for (const audit of needingSignatures) {
        ...
        await RampDb.transaction(async innerClient => {
...
export interface ISidechainAudit {
  auditDate: Date;
